...
export function useVaultState() {
  ...
  const [vaultSetupForm, setVaultSetupForm] = useState<VaultSetupForm>(createVaultSetupForm)
  const [vaultSetupBusy, setVaultSetupBusy] = useState(false)
  const [accessPassword, setAccessPassword] = useState('')
  const [accessBusy, setAccessBusy] = useState(false)
  const [changeMasterForm, setChangeMasterForm] = useState<ChangeMasterForm>(createChangeMasterForm)
